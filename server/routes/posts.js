...
const { publicPostFilter } = require('../utils/posts');

async function normalizePostInput(body) {
  const data = { ...body };
  if ('seo' in data) data.seo = normalizeSeo(data.seo);
  if ('displayPages' in data) data.displayPages = normalizePlacements(data.displayPages);
