...
import { ArrowDown, Camera, Maximize2, Play, Sparkles, Video, X } from 'lucide-react';
...
export default function WorkLifeClient({ initialItems = [] }) {
  ...
  return (
    <main className="worklife-page">
      <section className="worklife-hero">
        <div className="worklife-hero-media" aria-hidden="true">
          <video autoPlay muted loop playsInline preload="metadata">
            <source src="/worklife-hero.mp4" type="video/mp4" />
          </video>
        </div>
        <div className="worklife-hero-overlay" />
        <motion.div
          className="worklife-hero-glow worklife-hero-glow--one"
          animate={reduceMotion ? undefined : { x: [0, 24, 0], y: [0, -18, 0] }}
          transition={{ duration: 9, repeat: Infinity, ease: 'easeInOut' }}
        />
        <motion.div
          className="worklife-hero-glow worklife-hero-glow--two"
          animate={reduceMotion ? undefined : { x: [0, -20, 0], y: [0, 22, 0] }}
          transition={{ duration: 11, repeat: Infinity, ease: 'easeInOut' }}
        />
        <div className="container worklife-hero-inner">
          <motion.div
            className="worklife-hero-copy"
            initial={initial}
            animate="visible"
            variants={gridMotion}
          >
            <motion.span className="worklife-kicker" variants={cardMotion}><Sparkles size={15} /> WorkLife@Proowrx</motion.span>
            <motion.h1 variants={cardMotion}>Great work starts with a <span>great culture.</span></motion.h1>
            <motion.p variants={cardMotion}>Meet the people, celebrations and everyday moments that bring our offices to life.</motion.p>
            <motion.div className="worklife-hero-stats" variants={cardMotion}>
              <div><Camera size={18} /><strong>{images.length}</strong><span>Moments</span></div>
              <div><Video size={18} /><strong>{videos.length}</strong><span>Stories</span></div>
              <div><Sparkles size={18} /><strong>One</strong><span>Team</span></div>
            </motion.div>
            <motion.a href="#worklife-gallery" className="worklife-explore" variants={cardMotion}>
              Explore our culture <ArrowDown size={16} />
            </motion.a>
          </motion.div>
        </div>
      </section>
